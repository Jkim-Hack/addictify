...
class AuthService {
  ...
  LUser userValueFromFirebase(User user) {
    return LUser(uid: user.uid, name: user.displayName, email: user.email, coins: 20);
  }
  ...
  Future<LUser> registerWithEmailAndPassword(String email, String password) async
  {
    try {
      ...
      await UserService(signedInUser.uid).updateUser(LUser(uid:signedInUser.uid,name:"",email:email, coins: 20));
    ...
    catch(error)
    {
      ...
      return LUser(uid: "", name: "", email: "", coins: 0);
    }
  }
